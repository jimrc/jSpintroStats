...
// TODO: Find a better situation?  Problem: if we don't care about separating into
// two groups (based on lurking), then what is phat estimating? The proportion
// success in the overall population? or just in one group?
//  Proportion of 6's in dice rolls with possible weighted dice in the sample?
  ...
  if(typeof(plsvg) === "object"){
    ...
    pCIs = plsvg.selectAll("line")
       ...
       document.getElementById('lurkC1DemoResults').innerHTML =
         //" Smokers survival: " + trueP +
         //" - observed proportion ( " + (d3.mean(ciDemoLines, d => d.center)).toPrecision(3) " ) = " +
         "  Bias: " + (d3.mean(ciDemoLines, d => d.center) - trueP).toPrecision(3) + "<br>"+
          countr + " blue and " + (nreps - countr) +" red intervals. "+
