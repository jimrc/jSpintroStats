...
function CLChange(arg) {
  ...
  if (inferenceSVG.empty()) {
    makeScatterPlot(CIData, "infSVGplot", xLabel, xLab, " ", false);
  } else {
    // update Colors
  ...
  //makeScatterPlot(CIData, "infSVG", xLabel, xLab, " ", false);
...
function moreTests(nreps, concat) {
  ...
  if (nreps > 0) {
    ...
    case 'cat1': {
      ...
      observed = proportion;
      xLabel = "Proportions " + cat1Label1 + " from resamples under the null ";
      xLab = "Phat ";
      break;
    }
    case 'quant1':{
      ...
      observed = q1Xbar;
      xLabel = "Means from resamples under the null  ";
      xLab = "Resampled mean ";
      break;
    }
    case 'cat2': {
      ...
      nullValue = 0.0;
      observed = difference;
      xLabel = "Differences in proportions from resamples under the null ";
      xLab = "Phat1 - Phat2 ";
      break;
    }
    case 'c1q1': {
      ...
      nullValue = 0.0;
      observed = diff;
      xLabel = "Differences in means from resamples under the null ";
      xLab = "Resampled Mean1 - Mean2 ";
      break;
    }
    case 'quant2': {
      newSample = resampleSlope4Test(q2Values, nreps);
      xLabel = "Slopes from resampled data under the null ";
      xLab = "Resampled Slope";
      nullValue = 0.0;
      observed = slope;
      break;
...
case "both": {
  ...
  for (i = 0; i < sLen; i++) {
    check = 0.0 + (sample4Test[i] <= lowV)
    if (check == 0.0 & sample4Test[i] >= hiV) {
      check = 1.0
    };
    extCount += check;
    testColor[i] = check;
...
makeScatterPlot(testData, "infSVGplot", xLabel, xLab, " ", false);
