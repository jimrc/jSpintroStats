...
function makeScatterPlot(data, plot, xlabel, shortXlabel, ylabel, addLine) {
  ...
  data_extent = {
    max: {
      // assumes positive x and y values
      x: d3.max(x) * 1.01,
      y: d3.max(y) * 1.01
    },
    min: {
  ...
  x_scale = d3.scaleLinear().domain([data_extent.min.x, data_extent.max.x]).range([0, max_draw_space.x]);
  y_scale = d3.scaleLinear().domain([data_extent.max.y, data_extent.min.y]).range([0, max_draw_space.y]);
  ...
  var scatter_group = chart_group.selectAll("g." + plot + "_scatter_group").data(data, function (d, i) {
    return i;
  });
  chart_group
  ...
  if (addLine) {
    ...
    if (slope) {
       regLine = chart_group
        ...
        .attr("y1", y_scale(xmin * slope + intercept) + margins.y.top + margins.title)
  ...
  function mouseClickFunction(d, i) {
    ...
    make_tooltip(d3.select(this), [shortXlabel + ": " + d.x.toPrecision(3)], chart_group, canvas, margins);
  }
...
function calculate_space_needed_by_axes(chart_group, data_extent, margins) {
  ...
  x_scale = d3.scaleLinear().domain([data_extent.min.x, data_extent.max.x]).range([0, 1]);
  y_scale = d3.scaleLinear().domain([data_extent.min.y, data_extent.max.y]).range([0, 1]);

  // Make the axes
  var x_axis_scaled = d3.axisBottom(x_scale).tickSize(charts_config.axes.tickssize).ticks(6);
  var y_axis_scaled = d3.axisLeft(y_scale).tickSize(charts_config.axes.tickssize).ticks(6);
...
function make_axes(
  ...
  var x_axis_scaled = d3.axisBottom().scale(x_scale).tickSize(charts_config.axes.ticks.size).ticks(6);
  var y_axis_scaled = d3.axisLeft().scale(y_scale).tickSize(charts_config.axes.ticks.size).ticks(6);
  var center = {
  ...
  x_axis_label = axes
    ...
    .attr("transform", "translate(" + center.x + "," + (canvas.y - margins.y.bottom) + ")")
  ...
  y_axis_label = axes
    ...
    .attr("font-family", charts_config.axes.labels.family)
    .attr("font-size", charts_config.axes.labels.size)
    .attr("transform", "translate(" + margins.x.left + "," + center.y + ") rotate(-90)")
    .attr("text-anchor", "middle");
...
function make_title(chart_group, text_array, margins, canvas, maximum_drawing_space) {
  ...
  // Max line length
  var characters_per_line = (canvas.x - margins.x.right) / (charts_config.title.size / 2);
...
function make_tooltip(obj, text_array, chart_group, canvas, margins) {
  ...
  for (var i = 0; i < text_array.length; i++) {
    ...
    if (i < text_array.length - 1) {
      text.attr("fill", tooltip_config.text).attr("font-family", tooltip_config.family).attr("font-weight", "normal");
    }
  }
  ...
  for (var ii = 0; ii < placements.length; ii++) {
  ...
  tooltip.attr(
    "transform",
    "translate(" + (tooltip_placement.x - bubble_box.x) + "," + (tooltip_placement.y - bubble_box.y) + ")"
  );
}
