...
//  max_draw_space = calculate_maximum_drawing_space(canvas, margins)
  ...
  function mouseOutFunction(d, i) {
   chart_group.select("g.tooltip_group").remove()
    ...
    var regLine = chart_group.append("line")
...
function calculate_space_needed_by_axes(chart_group, data_extent, margins) {
  ...
  // Make the axes
  var x_axis_scaled = d3.axisBottom().scale(x_scale).tickSize(charts_config.axes.tickssize).ticks(6)
  var y_axis_scaled = d3.axisLeft().scale(y_scale).tickSize(charts_config.axes.tickssize).ticks(6)

  // Place and maintain the axis as an SVG object
    var x_axis= temp.append("g").attr("class", "temp")
	.call(x_axis_scaled).attr("font-size", charts_config.axes.size)
	.attr("font-family", charts_config.axes.family)
    var y_axis= temp.append("g").attr("class", "temp")
	.call(y_axis_scaled)
	.attr("font-size", charts_config.axes.size)
	.attr("font-family", charts_config.axes.family)

  // Add the axes labels. y-axis is rotated
    var x_axis_label = temp.append("text").text("temp")
	.attr("font-family", charts_config.axes.labelsfamily)
	.attr("font-size", charts_config.axes.labelsize)
    var y_axis_label = temp.append("text").text("temp")
	.attr("font-family", charts_config.axes.labelsfamily)
	.attr("font-size", charts_config.axes.labelsize)
...
function make_axes(chart_group, x_scale, y_scale, canvas, margins, maximum_drawing_space, x_label, y_label, custom_ticks=false) {
  ...
  axes = chart_group.select("g.axes")
    var x_axis_scaled = d3.axisBottom().scale(x_scale)
	.tickSize(charts_config.axes.ticks.size).ticks(6)
    var y_axis_scaled = d3.axisLeft()
	.scale(y_scale).tickSize(charts_config.axes.ticks.size).ticks(6)
    var center = {"x": margins.x.left + margins.axes.y +
		  maximum_drawing_space.x / 2,
		  "y": margins.y.top  + margins.title +
		  maximum_drawing_space.y / 2}
  ...
  if (custom_ticks!=false) {
    ...
    x_axis_label = axes.select("text.x_axis_label").text(x_label)
	...
	.attr("transform", "translate(" + center.x + "," +
	      (canvas.y - margins.y.bottom)+")")
	.attr("text-anchor", "middle")
    y_axis_label = axes.select("text.y_axis_label").text(y_label)
	.attr("font-family", charts_config.axes.labels.family)
	.attr("font-size", charts_config.axes.labels.size)
	.attr("transform", "translate(" + margins.x.left + "," + center.y + ") rotate(-90)").attr("text-anchor", "middle")
